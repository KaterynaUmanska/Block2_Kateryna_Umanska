...
Ендпоінт списку записів про закупівлі підтримує необов'язкові фільтри:
...
## Звіт
Ендпоінт звіту використовує ті самі критерії фільтрації, що й кінцева точка списку.
...
## Робота з CSV-файлом
Для коректного відображення української мови та кирилиці при відкритті згенерованого CSV-файлу в Microsoft Excel треба використовувати імпорт через меню `Дані` далі `З тексту/CSV` із вибором кодування `UTF-8`, оскільки файл містить BOM-маркер.
...
Для оптимізації пошуку, підтримки зв'язків та забезпечення унікальності даних у схемі бази даних створено такі індекси:
* purchase_records.order_id.
* purchase_records.material_id.
* unique (order_id, material_id).
* uk_material_name_lower.

Окремий індекс для стовпця quantity не створювався.
