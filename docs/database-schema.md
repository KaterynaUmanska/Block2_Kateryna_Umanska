...
Поле quantity використовується для необов'язкової фільтрації за діапазоном. 
Додатковий індекс не потрібен для поточного навантаження на додаток і обсягу даних. 
...
* Перед збереженням PurchaseRecord додаток перевіряє, чи існує вказаний Material.
* Запис про закупівлю зберігається в таблицю purchase_records.
* Під час отримання списку необов'язкові фільтри застосовуються безпосередньо на рівні запиту до бази даних.
* Пагінація також виконується на рівні бази даних за допомогою Pageable. 
* Звіти використовують ту саму логіку фільтрації та містять усі записи про закупівлі, що відповідають заданим критеріям. 
* Під час імпорту JSON кожен запис окремо парситься та валідується. Додатково перевіряється існування пов'язаного матеріалу та виконуються бізнес-перевірки. Валідні записи зберігаються, а помилки окремих записів не перешкоджають обробці інших.
